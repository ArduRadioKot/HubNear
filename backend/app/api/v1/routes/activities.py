...
def build_viewer_state(row: dict, viewer_user_id: UUID | None) -> ActivityViewerState:
    ...
    if viewer_user_id is None:
        return ActivityViewerState(
            ...
            role=None,
            can_join=False,
            join_block_reason="auth_required",
        )
    ...
    if row["status"] not in ("open", "confirmed"):
        return ActivityViewerState(
            ...
            role=None,
            can_join=False,
            join_block_reason="activity_closed",
        )
    ...
    if deadline < now:
        return ActivityViewerState(
            ...
            role=None,
            can_join=False,
            join_block_reason="deadline_passed",
        )
    ...
    if row["current_participants_count"] >= row["max_participants"]:
        return ActivityViewerState(
            ...
            role=None,
            can_join=False,
            join_block_reason="activity_full",
        )
...
def activity_from_row(row: dict, viewer_user_id: UUID | None) -> ActivityDetail:
    ...
    return ActivityDetail(
        ...
        participants=ActivityParticipantsSummary(
            count=row["current_participants_count"],
            minimum=row["min_participants"],
            limit=row["max_participants"],
        ),
        price=price,
        status=row["status"],
...
def activity_list_item_from_row(row: dict, viewer_user_id: UUID | None) -> ActivityListItem:
    return ActivityListItem.model_validate(activity_from_row(row, viewer_user_id).model_dump())
...
def raise_activity_write_error(error: IntegrityError) -> None:
    ...
    if sqlstate == "23503":
        raise_api_error(
            ...
            "Related user, city, or activity not found",
        )
    ...
    if sqlstate == "23514":
        if message == "activity is full":
            raise_api_error(status.HTTP_409_CONFLICT, "activity_full", "Activity is full")
        if message == "activity is closed or expired":
            raise_api_error(status.HTTP_409_CONFLICT, "activity_closed", "Activity is closed or expired")
        if message == "activity is deleted":
            raise_api_error(status.HTTP_409_CONFLICT, "activity_deleted", "Activity is deleted")
        raise_api_error(
            status.HTTP_409_CONFLICT,
            "activity_invalid_state",
            message or "Activity state does not allow this action",
        )
...
async def list_activities(
    ...
    if radius_m is not None and not has_origin:
    ...
    result = await connection.execute(
        text(
            f"""
            {activity_select(distance_expression)}
...
async def leave_activity(
    ...
    if deleted_user_id is None:
        raise_api_error(
            status.HTTP_409_CONFLICT,
            "cannot_leave_activity",
            "Participant not found or organizer cannot leave",
        )
